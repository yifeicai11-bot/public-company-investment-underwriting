...
DEFAULT_OUT_ROOT = ROOT / "partner-demo" / "investment_decision_v2" / "final_delivery"
...
def report_header(contract: dict[str, Any], subtitle: str) -> str:
    ...
    return f"""
<header class="header"><div><div class="eyebrow">{esc(contract.get('product_positioning') or subtitle)}</div>
...
def headline_metrics(contract: dict[str, Any]) -> str:
    ...
    liquidity_label = (
        ...
        if gross_liquidity
        else "Cash + short-term investments / 现金及短期投资"
    )
    liquidity_note = (
...
def qa_summary_html(contract: dict[str, Any]) -> str:
    ...
    body = report_header(contract, "Friday V1 QA Summary / Friday V1质量检查摘要")
    body += decision_strip(contract)
    body += (
    ...
    body += running_footer(contract)
    return html_page(
        f"{contract.get('company', {}).get('ticker')} Friday V1 QA Summary / 质量检查摘要",
        body,
        one_page=True,
    )
...
def render(contract_path: Path, out_dir: Path, *, pdf: bool = False) -> dict[str, Any]:
    ...
    if formal_blocked:
        ...
        qa_summary = out_dir / f"{ticker}_Friday_V1_QA_Summary_Bilingual.html"
        ...
        if pdf:
            ...
            qa_pdf = out_dir / f"{ticker}_Friday_V1_QA_Summary_Bilingual.pdf"
