...
def markdown_reports(contract: dict[str, Any]) -> dict[str, str]:
    ...
    partner = contract["partner_decision"]
    ...
    snapshot_rows = [
        ...
        ["Partner Decision / Partner 决策", decision],
    ...
    for row in contract.get("constraint_snapshot", {}).get("constraints", []):
        constraint_rows.append(
            [
                ...
                row.get("label_zh"),
                row.get("status"),
                _fmt_pct(row.get("limit_value")),
                _fmt_pct(row.get("current_value")),
                _fmt_pct(row.get("candidate_value")),
    ...
    approved_range = partner.get("approved_position_range")
    approval_lines = [
        f"- Submitted decision / 提交决定: {partner.get('submitted_decision')}",
        f"- Effective decision / 生效决定: {partner.get('decision')}",
        f"- Validation / 验证: {partner.get('validation_status')}",
    ...
    full_report = "\n".join(
        [
            ...
            *[f"- {value}" for value in assessment.get("rationale_en", [])],
            *[f"- {value}" for value in assessment.get("rationale_zh", [])],
            f"- Can support Partner approval / 可支持 Partner 审批: {assessment.get('can_support_partner_approval')}",
            "",
...
def one_page_html(contract: dict[str, Any]) -> str:
    ...
    body += (
        "<section class='panel'><h2>Partner Decision / Partner 决策</h2>"
        f"<p><b>Submitted / 提交:</b> {_esc(partner.get('submitted_decision'))} &nbsp; "
        f"<b>Effective / 生效:</b> {_esc(partner.get('decision'))} &nbsp; "
        f"<b>Validation / 验证:</b> {_esc(partner.get('validation_status'))}</p>"
        "</section>"
    )
    body += (
...
def _constraint_table(contract: dict[str, Any], *, include_formula: bool) -> str:
    ...
    for row in contract.get("constraint_snapshot", {}).get("constraints", []):
        values = [
            ...
            _esc(row.get("status")),
            _esc(_fmt_pct(row.get("limit_value"))),
            _esc(_fmt_pct(row.get("current_value"))),
            _esc(_fmt_pct(row.get("candidate_value"))),
...
def _constraint_audit_cards(contract: dict[str, Any]) -> str:
    ...
    for row in contract.get("constraint_snapshot", {}).get("constraints", []):
        metrics = [
            ...
            ("Limit / 上限", _fmt_pct(row.get("limit_value"))),
            ("Current / 当前", _fmt_pct(row.get("current_value"))),
            ("Candidate / 候选", _fmt_pct(row.get("candidate_value"))),
...
def full_report_html(contract: dict[str, Any]) -> str:
    ...
    body += (
        ...
        f"<p><b>Partner:</b> {_esc(partner.get('decision'))} ({_esc(partner.get('validation_status'))})</p>"
        "<p>The system does not own the Partner decision. / 系统不拥有 Partner 的最终决定。</p></section></div>"
    )
    ...
    body += "<h2>Partner Decision Record / Partner 决策记录</h2>"
    approved_range = partner.get("approved_position_range")
    body += (
        "<table><tbody>"
        f"<tr><th>Submitted / 提交</th><td>{_esc(partner.get('submitted_decision'))}</td></tr>"
        f"<tr><th>Effective / 生效</th><td>{_esc(partner.get('decision'))}</td></tr>"
        f"<tr><th>Validation / 验证</th><td>{_esc(partner.get('validation_status'))}</td></tr>"
        ...
        f"<tr><th>Blocking reasons / 阻断原因</th><td>{_esc(', '.join(partner.get('blocking_reason_codes', [])) or 'None / 无')}</td></tr>"
        "</tbody></table>"
    )
    body += (
