...
def build_shared_valuation_contract(
    ...
    partner_internal_return = {
        ...
        "reason": (
            "Partner internal return belongs to the repo-external private Gate 4 overlay and is never "
...
def validate_shared_valuation_contract(contract: dict[str, Any]) -> list[str]:
    ...
    partner = outputs["partner_internal_return"]
    if partner.get("status") != "DISABLED_PRIVATE_GATE_4_ONLY":
        errors.append("Partner internal return must remain disabled in the public issuer contract.")
    for field in ("expected_return", "target_return", "portfolio_hurdle", "position_sizing"):
        if partner.get(field) is not None:
