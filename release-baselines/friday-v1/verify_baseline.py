#!/usr/bin/env python3
"""Verify and optionally regenerate the frozen Friday V1 release baseline."""
...
def regenerate(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="friday-v1-baseline-") as temporary:
        temporary_root = Path(temporary)
        tagged_root = temporary_root / "tagged-source"
        tagged_root.mkdir()
        extract_tag(str(manifest["tag"]), tagged_root)

        renderer = tagged_root / "partner-demo/investment_decision_v2/scripts/render_public_company_artifacts.py"
        validator = tagged_root / "partner-demo/investment_decision_v2/scripts/validate_friday_v1_delivery.py"
        for ticker, case in manifest["cases"].items():
            contract = tagged_root / case["contract_path"]
            ...
            html_names = [
                ...
                f"{ticker}_Friday_V1_QA_Summary_Bilingual.html",
            ]
            for name in html_names:
                expected = expected_dir / name
                actual = regenerated_dir / name
                record(
                    checks,
...
def main() -> None:
    ...
    if sys.version_info < (3, 11):
        raise SystemExit("Friday V1 requires Python 3.11 or newer.")
