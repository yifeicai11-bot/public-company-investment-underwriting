...
6. Read `references/filing_note_extraction.md` when any module is material, Medium/High, or dependent on note-level disclosure. Debt, revolver/ABL, maturity, lease, covenant, acquisition, and refinancing conclusions require note-level reading. Before changing or reproducing note/event classifications, also read `references/notes_and_events_controls.md`; its safe-status, revolver, acquisition, source-conflict, freshness, amendment, restatement, supplier-finance, and subsequent-event rules are binding.
