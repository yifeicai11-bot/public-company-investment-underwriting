...
for the local portfolio overlay. It contains no real fund or User data.
...
| Approval config | JSON or YAML | Separates system assessment from the User-owned decision and disables automatic trading |
...
## What Requires Portfolio Data / 哪些功能需要组合数据

Gate 3 and Gate 4 answer different questions:

- Gate 3 asks whether the issuer and its public-data valuation/scenario case
  are sufficiently validated for an issuer-level investment review.
- Gate 4 asks whether that candidate fits a specific fund's limits, existing
  exposures, liquidity requirements, risk budget, opportunity set, and
  User approval process.

Therefore, a user who does not want to provide portfolio data can still run
the complete public-only Gate 3 workflow. The result keeps `Portfolio Overlay:
Disabled`, `Portfolio Decision: Not Evaluated`, and position sizing disabled.
Gate 4 cannot infer fund-specific constraints from the ticker alone.

The minimum private input burden depends on the selected mode. `EXPOSURE_ONLY`
is the least granular option, but it cannot support holdings reconciliation or
security-level liquidity. `AGGREGATED_PORTFOLIO` supports issuer and aggregate
portfolio checks. `FULL_HOLDINGS` supports the most complete constraint
calculation, but still does not allow the system to own the final User
decision. The synthetic examples in this directory demonstrate the interface
only; they must not be treated as real portfolio information.
...
invalid until the User completes them locally.
...
- `PARTNER_APPROVAL_PENDING`: no User decision may be treated as approved before the system assessment.
- `GATE_4_APPROVED`: a named User decision is validated against the current assessment hash and constraint ceiling.
- `GATE_4_MODIFIED`, `GATE_4_REJECTED`, and `GATE_4_DEFERRED`: other validated User-owned outcomes.
...
`NOT_EVALUATED`. The separately owned User decision may be `PENDING`,
...
```bash
python3 user-demo/investment_decision_v2/scripts/validate_gate4_private_inputs.py \
  user-demo/investment_decision_v2/gate4/synthetic_examples/synthetic_gate4_manifest.json

python3 user-demo/investment_decision_v2/scripts/validate_gate4_private_inputs.py \
  user-demo/investment_decision_v2/gate4/synthetic_examples/synthetic_aggregated_portfolio_manifest.json

python3 user-demo/investment_decision_v2/scripts/validate_gate4_private_inputs.py \
  user-demo/investment_decision_v2/gate4/synthetic_examples/synthetic_exposure_only_manifest.json
```
