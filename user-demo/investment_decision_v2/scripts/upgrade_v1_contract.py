#!/usr/bin/env python3
"""Upgrade an audited underwriting contract to the shared v1.0.0 schema.
...
from build_public_company_investment_layer import apply_v1_0_0_contract_semantics
...
def upgrade(contract_path: Path, research_input_path: Path, out_dir: Path) -> Path:
    ...
    upgraded = apply_v1_0_0_contract_semantics(contract, research_input)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Upgrade an audited public-company contract to v1.0.0 without refetching data."
    )
