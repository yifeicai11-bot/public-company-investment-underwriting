#!/usr/bin/env python3
"""Shared S14 Gate 4 assessment and User-decision engine.

The engine consumes a validated S13 result. It classifies portfolio eligibility,
validates a separately owned User decision, and never selects a position or
places a trade.
"""
...
def _s13_hash_payload(result: dict[str, Any]) -> dict[str, Any]:
    ...
    # invalidate a User decision bound to otherwise identical inputs.
    ...
    for escalation_id in escalation_ids:
        checks.append(
            _check(
                ...
                message_fail_en=f"Active escalation requires User acknowledgement: {escalation_id}.",
                message_fail_zh=f"有效升级项需要 User 明确认可：{escalation_id}。",
    ...
    *,
    ...
    checks.append(
        _check(
            "S14-USER-DECISION",
            valid,
            message_pass_en="The recorded User decision is complete and consistent with the current assessment.",
            message_pass_zh="已记录的 User 决策完整且与当前评估一致。",
            message_fail_en=(
                "The recorded User decision is blocked: " + ", ".join(blocking)
            ),
            message_fail_zh="已记录的 User 决策未通过验证。",
            evidence_ids=["G4-EV-USER-DECISION"],
        )
    )
    if not valid:
        return {
    ...
    *,
    ...
    evidence = [
        ...
        },
        {
            ...
            "evidence_class": "CONTROL",
            "label_en": "Assessment-input fingerprint excluding the User decision",
            "label_zh": "不含 User 决策的评估输入指纹",
    ...
    evidence.extend(
        [
            ...
            {
                "evidence_id": "G4-EV-USER-DECISION",
                "evidence_class": "CONTROL",
                "label_en": "Separately owned User decision",
                "label_zh": "独立归属的 User 决策",
...
def build_gate4_assessment(
    ...
    *,
    ...
    if system_assessment["assessment"] == "NOT_EVALUATED":
        ...
        next_action_en = "The designated User should review the assessment and record a dated decision locally."
        next_action_zh = "应由指定 User 在本地复核评估并记录带日期的决定。"
    elif decision in {"APPROVED", "MODIFIED"}:
        next_action_en = "Apply the validated User-owned range through the fund's separate implementation controls; no trade is automatic."
        next_action_zh = "通过基金独立的执行控制落实经验证的 User 区间；系统不会自动交易。"
