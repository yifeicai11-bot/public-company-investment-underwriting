...
DEFAULT_OUT_ROOT = ROOT / "user-demo" / "investment_decision_v2" / "generic_outputs"
...
INSTANT_TAGS: dict[str, tuple[str, ...]] = {
    ...
    "marketable_equity_securities_current": (
...
CALCULATION_FORMULAS: dict[str, str] = {
    "total_liabilities": "total_assets - shareholders_equity",
    "available_liquidity_before_facility_notes": "unrestricted_cash + short_term_investments + marketable_equity_securities_current",
    "current_lease_obligations_total": "finance_lease_current + operating_lease_current",
    "latest_ytd_fcf": "latest_ytd_cfo - latest_ytd_capex",
    "latest_annual_fcf": "latest_annual_cfo - latest_annual_capex",
    "derived_latest_quarter_fcf": "derived_latest_quarter_cfo - derived_latest_quarter_capex",
...
def build_company_pack(query: str, out_root: Path = DEFAULT_OUT_ROOT) -> Path:
    ...
    marketable_equity = val(m, "marketable_equity_securities_current")
    liquid_rows = [
        m.get(name)
        for name in (
            "unrestricted_cash",
    ...
    observed_liquid_inputs = [
        value for value in (cash, sti, marketable_equity) if value is not None
    ]
    if observed_liquid_inputs and liquid_basis["status"] == "PASS":
        rows.append(
            manual_dp(
                ...
                filing_type=filing_for_period.form,
                filing_date=filing_for_period.filed,
                source_location="cash + current marketable securities",
                source_tag="calculation",
                source_url=latest_url,
                notes=(
                    "Sum of separately identified unrestricted cash, current debt securities, "
                    "and current marketable equity securities; excludes facility availability "
                    "until note review."
                ),
            )
        )
