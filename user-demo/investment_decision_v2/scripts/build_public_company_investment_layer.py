...
def build_valuation(
    ...
    if cash_sti is None:
        ...
        marketable_equity = metric_value(step2_metrics, "marketable_equity_securities_current") or 0.0
        cash_sti = unrestricted_cash + short_term_investments + marketable_equity
...
def _migrate_v1_0_0_evidence_semantics(contract: dict[str, Any]) -> None:
...
def apply_v1_0_0_contract_semantics(
    contract: dict[str, Any],
    research_input: dict[str, Any],
) -> dict[str, Any]:
    """Apply shared v1.0.0 decision-boundary semantics to any company contract."""
    ...
    _migrate_v1_0_0_evidence_semantics(contract)
    if (
    ...
    what_cannot_be_concluded = [
        ...
        "User Internal Return or a portfolio action without the private Gate 4 overlay. / 缺少私有Gate 4叠加层时的User内部回报或组合动作。",
    ]
    ...
    contract.update(
        {
            "product_positioning": "Public-Data Issuer Underwriting and IC Pre-Read System - v1.0.0",
...
def build_system_validation_gates(
    ...
    gates.append(
        ValidationGate(
            ...
            "Approve the provider for research use or reconcile to a user-approved production feed.",
    ...
    gates.append(
        ValidationGate(
            ...
            "Complete the user overlay and obtain human approval.",
            "WARNING",
            "portfolio",
        )
    )
...
def build_validation_gates(
    ...
    gates.append(
        ValidationGate(
            ...
            "Complete user portfolio overlay before turning research-support action into portfolio action.",
        )
    )
    return gates
...
def final_action_view(gates: list[ValidationGate], scenarios: list[Scenario], scenario_status: str) -> tuple[str, str, str, str]:
    ...
    if expected >= 0.15 and bear >= -0.30 and bull >= 0.25:
        return (
            ...
            "No position sizing until user portfolio overlay is completed",
            "Medium",
...
def build_committee_roles(
    ...
    return [
        ...
        CommitteeRole(
            ...
            evidence="Scenario assumptions are public-data derived; user portfolio context is still a blocked validation gate.",
...
def valuation_rows(valuation: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        {"metric": "Cash + current marketable securities", "value": valuation.get("cash_and_short_term_investments"), "display": fmt_usd(valuation.get("cash_and_short_term_investments")), "use": "Liquid resources before facility note review"},
...
def build_markdown(
    ...
    lines = [
        ...
        "- It does not support final buy/sell/hold or position sizing until user portfolio context and analyst-reviewed assumptions are added.",
        "",
        ...
        *scenario_table,
        ...
        "- The public-data model can support underwriting priority, but final action and sizing remain blocked until analyst review and user portfolio overlay are complete.",
        "",
        ...
        *gate_table,
        ...
        "- Use `portfolio_overlay_template.csv` or `portfolio_overlay_template.md` in this folder to add target return, downside tolerance, existing exposure, sizing range, and opportunity-cost alternatives.",
...
def partner_overlay_markdown(company: dict[str, Any]) -> str:
    lines = [
        f"# {company['name']} ({company['ticker']}) User Portfolio Overlay Template",
        "",
        ...
        "",
        "| Field | Example / Default | Why Needed | Sensitivity | User Input |",
        "|---|---|---|---|---|",
    ]
...
def build_unsupported_investment_output(step2: dict[str, Any], out_dir: Path) -> Path:
    ...
    payload = apply_v1_0_0_contract_semantics(payload, {})
...
def build_investment_layer(
    ...
    write_dict_csv(
        step3_dir / "portfolio_overlay_template.csv",
        partner_overlay_rows(),
        ["field", "example_or_default", "why_needed", "sensitivity", "partner_input"],
    )
    (step3_dir / "portfolio_overlay_template.md").write_text(partner_overlay_markdown(company), encoding="utf-8")
    ...
    payload = apply_v1_0_0_contract_semantics(payload, research_input)
    payload = finalize_output_contract(payload)
    serialized = json.dumps(payload, indent=2, ensure_ascii=False, default=str)
...
def main() -> None:
    ...
    for name in (
        ...
        "portfolio_overlay_template.csv",
        "portfolio_overlay_template.md",
