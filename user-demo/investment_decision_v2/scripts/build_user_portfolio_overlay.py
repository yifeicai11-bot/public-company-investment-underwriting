...
RATIONALE_ZH = {
    ...
    "User overlay clears return, downside, variant, catalyst, and sizing gates.": "叠加输入通过回报、下行、差异化观点、催化剂和仓位约束检查。",
    "Expected return fails hurdle and bear case exceeds downside tolerance.": "预期回报低于门槛，且熊市情景下行超过可接受范围。",
    "Expected return does not clear user hurdle.": "预期回报没有达到 user 设定的门槛。",
    "Bear-case downside is outside user tolerance.": "熊市情景下行超出 user 可接受范围。",
...
def overlay_gates(
    ...
    gates.append(
        OverlayGate(
            ...
            "Expected return must clear the user's hurdle before portfolio action.",
    ...
    gates.append(
        OverlayGate(
            ...
            "Sizing cannot be suggested without a user-provided size constraint.",
...
def choose_overlay_action(gates: list[OverlayGate], overlay: dict[str, Any], returns: dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    if not return_pass:
        return (
            ...
            "No position sizing",
            "Low",
            "Expected return does not clear user hurdle.",
        )
    if not downside_pass:
        return (
            ...
            "No position sizing",
            "Low",
            "Bear-case downside is outside user tolerance.",
        )
    return (
...
def build_markdown(
    ...
    lines = [
        f"# {company['name']} ({company['ticker']}) Public-Only and User Overlay Demo / 公开数据与 User 组合叠加演示",
        "",
        f"Generated: {utc_now()}",
        ...
        "",
        "This output combines the public-data Step 3 memo with a user/portfolio overlay. If `overlay_mode` is `ILLUSTRATIVE_DEMO_NOT_FUND_DATA`, the overlay inputs are examples for workflow demonstration, not real fund data.",
        "",
        "本文件把 Step 3 公开数据分析和 user/组合约束叠加在一起。如果 `overlay_mode` 是 `ILLUSTRATIVE_DEMO_NOT_FUND_DATA`，说明这些 overlay 输入只是流程演示，不是真实基金数据或 user 观点。",
        "",
        ...
        *row_table(
        ...
        "",
        f"- Public-only answer: {step3.get('action_view')} with no sizing because user portfolio context was missing.",
        f"- 公开数据结论：{zh_action(step3.get('action_view'))}；由于缺少 user/组合约束，不能给出仓位建议。",
        f"- Overlay answer: {action}; sizing remains controlled by user input and is not a formal trade instruction.",
        f"- 叠加后结论：{zh_action(action)}；仓位仍由 user 输入控制，不是正式交易指令。",
        ...
        "- Overlay input: user-provided or illustrative JSON/CSV.",
        "- Overlay 输入：user 提供或用于演示的 JSON/CSV。",
        "",
    ]
    return "\n".join(lines)
