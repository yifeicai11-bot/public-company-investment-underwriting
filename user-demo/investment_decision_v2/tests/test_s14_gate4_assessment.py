...
class S14Gate4AssessmentTests(unittest.TestCase):
    ...
    def write_decision(
        ...
        *,
        ...
        approved_by: str = "Synthetic User",
    ) -> None:
    ...
    def test_constraint_report_preserves_days_and_currency_units(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = self.copy_workspace(Path(tmp))
        ...
        full_report = html_reports(result)["full_report"]
        self.assertIn("365 days", full_report)
        self.assertIn("50,000,000 (base currency)", full_report)
        self.assertNotIn("36500%", full_report)
        self.assertNotIn("5000000000.0%", full_report)
        ...
        for status in ("REJECTED", "DEFERRED"):
            with self.subTest(status=status):
                decision, _ = build_partner_decision(
                    {
                        "designated_partner": "Synthetic User",
                        "partner_decision": {
                            "status": status,
