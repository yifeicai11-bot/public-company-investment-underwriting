...
from build_user_portfolio_overlay import (  # noqa: E402
    build_overlay,
...
class AccountingControlTests(unittest.TestCase):
    ...
    def test_current_marketable_equity_securities_have_a_separate_liquidity_metric(self) -> None:
        self.assertIn(
            "EquitySecuritiesFvNi",
            INSTANT_TAGS["marketable_equity_securities_current"],
        )
...
class Gate4EligibilityTests(unittest.TestCase):
    ...
    def test_warning_code_is_valid_fallback_identifier_for_gate4(self) -> None:
        contract = copy.deepcopy(self.contract)
        contract["warnings"] = []
        contract["validation_issues"] = [
            {
                "code": "TEST_WARNING_CODE_ONLY",
                "status": "WARNING",
                "issue_class": "WARNING",
                "category": "issuer_underwriting",
                "message": "Synthetic warning used to test stable identifier fallback.",
                "decision_impact": "The warning must remain visible to Gate 4.",
                "remediation": "Review or escalate the warning.",
            }
        ]
        ...
        attestation = self.attestation(
            ...
            warning_escalations=[
                {
                    "check_id": "TEST_WARNING_CODE_ONLY",
                    "reviewed_by": "Eligibility test reviewer",
                    "review_date": "2026-07-17",
                    "rationale": "Accepted for the stable identifier fallback test.",
                }
            ],
        )
        result = assess_gate3_for_gate4(
        ...
        self.assertEqual(result["status"], "GATE_4_PRIVATE_INPUTS_REQUIRED")
        self.assertIn("TEST_WARNING_CODE_ONLY", result["escalated_warning_ids"])
        self.assertNotIn("G4E-warning:missing-id", result["blocking_check_ids"])
...
class RenderingContractTests(unittest.TestCase):
    ...
    def test_headline_metrics_uses_selected_multiple_and_cash_fallback(self) -> None:
        ...
        self.assertIn("Cash + current marketable securities", rendered)
...
class V1_0_0RegressionTests(unittest.TestCase):
    @staticmethod
    def _contract(company_slug: str) -> dict[str, object]:
        path = TEST_DIR.parent / "v1_0_0_outputs" / company_slug / "step3" / "underwriting_output_contract.json"
    ...
    def test_evidence_aliases_are_complete_and_raw_ids_are_separate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = TEST_DIR.parent / "v1_0_0_outputs" / "crox_crocs_inc" / "step3" / "underwriting_output_contract.json"
    ...
    def test_one_page_and_full_report_have_shared_contract_identity(self) -> None:
        contract = self._contract("azo_autozone_inc")
        with tempfile.TemporaryDirectory() as tmp:
            source = TEST_DIR.parent / "v1_0_0_outputs" / "azo_autozone_inc" / "step3" / "underwriting_output_contract.json"
