...
from build_user_portfolio_overlay import return_pack  # noqa: E402
from validate_v1_delivery import validate_delivery  # noqa: E402
...
class SharedValuationContractTests(unittest.TestCase):
    ...
    def test_private_partner_return_is_discarded_from_public_contract(self) -> None:
        ...
        result = build_shared_valuation_contract(parent, supplied)
        user = result["outputs"]["partner_internal_return"]
        self.assertTrue(user["private_input_detected_and_discarded"])
        self.assertIsNone(user["target_return"])
        self.assertIsNone(user["position_sizing"])
    ...
    def test_renderer_displays_four_classes_without_recalculation(self) -> None:
        ...
        self.assertIn("User Internal Return", html)
